...
import { toIndiaDateTime } from '../../../../utils/helper'
...
export default function BetDetailsModal({ isOpen, onClose, bet }: BetDetailsModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 backdrop-blur-sm">
      <div className="bg-white rounded-xl shadow-xl p-4 w-full max-w-3xl max-h-[500px] overflow-y-auto border border-gray-100 transform transition-all duration-300 scale-100">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Complete bidding information</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2 mb-2">
          <div className="relative overflow-hidden bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-5 text-white shadow-lg">
            ...
            <p className="text-blue-100 text-xs font-medium mb-1">Agent Name</p>
            <p className="font-bold text-lg">{bet.full_name}</p>
          </div>
          ...
          <div className="relative overflow-hidden bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl p-5 text-white shadow-lg">
            ...
            <p className="text-purple-100 text-xs font-medium mb-1">Bid ID</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-1">
          <div className="text-center p-5 bg-white rounded-xl border-2 border-gray-100 hover:border-gray-200 transition-colors shadow-sm">
            <p className="text-xs text-gray-500 font-medium mb-2">Bid Type</p>
            <p className="font-bold capitalize text-gray-900 text-lg">{bet.bid_type_name}</p>
          </div>

          <div className="text-center p-5 bg-white rounded-xl border-2 border-gray-100 hover:border-gray-200 transition-colors shadow-sm">

            <p className="text-xs text-gray-500 font-medium mb-2">Number</p>
            <p className="font-mono font-bold text-2xl bg-gradient-to-r from-orange-500 to-red-500 bg-clip-text text-transparent">{bet.bid_number}</p>
          </div>
          ...
          <div className="text-center p-5 bg-white rounded-xl border-2 border-gray-100 hover:border-gray-200 transition-colors shadow-sm">
            ...
            <span className={`px-4 py-2 text-sm capitalize font-bold rounded-full ${getStatusColor(bet.status)}`}>
              {bet.status}
            </span>
          </div>
        </div>
        <div className="bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl p-6 border border-gray-200">
          <div className="flex items-center mb-3">
            ...
            <p className="text-sm text-gray-600 font-semibold">Bid Timestamp</p>
          </div>
          <p className="font-bold text-lg text-gray-900">{ moment(bet?.bid_date).tz("Asia/Kolkata").format("DD/MM/YYYY")}</p>
        </div>

        <div className="mt-4 flex justify-end">
          <button
            onClick={onClose}
            className="px-4 py-2 bg-gradient-to-r from-red-500 to-red-600 text-white rounded-xl hover:from-red-600 hover:to-red-700 transition-all duration-200 font-semibold shadow-lg hover:shadow-xl transform hover:scale-105"
          >
            Close Details
          </button>
        </div>
