...
export default function AllGamesPage() {
  ...
  const fetchGames = async () => {
    try {
      setLoading(true)
      const response = await apiClient.games.getAll()
      console.log('Fetched games:', response.data.games)
      ...
      {viewMode === 'table' ? (
        ...
        actions={(game) => (
          <div className="flex items-center justify-end space-x-4">
            ...
            <DeleteButton size="small" onClick={() => console.log('Delete', game.id)} />
          </div>
        )}
        />
      ) : (
          ...
          {games.filter(game => {
            ...
            <div key={game.id} className="bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 border border-gray-100 overflow-hidden">
              ...
              <div className={`h-2 bg-gradient-to-r ${
                game.status === 'active' ? 'from-green-400 to-green-600' : 'from-blue-500 to-orange-500'
              }`}></div>
              ...
              <div className="p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex flex-col items-end">
                    <span className={`px-3 py-1 text-xs font-bold rounded-full capitalize tracking-wide ${
                      game.status === 'active' ? 'bg-red-100 text-green-700 border border-green-200' : 'bg-red-100 text-red-700 border border-red-200'
                    }`}>
                      {game.status}
                    </span>
                ...
                <div className="flex items-center justify-center space-x-2">
                  ...
                  <button className="flex-1 bg-red-50 hover:bg-red-100 text-red-600 p-2 rounded-lg transition-colors duration-200 flex items-center justify-center">
