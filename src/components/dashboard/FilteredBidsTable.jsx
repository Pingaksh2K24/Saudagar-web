...
export default function FilteredBidsTable({
  adminBids,
  pagination = {},
  onPageChange,
}) {
  ...
  const totalPages =
    pagination.total_pages || Math.ceil(totalRecords / recordsPerPage);
  ...
  return (
    <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-gray-800">Filtered Bids</h3>
      ...
      <div className="grid grid-cols-3 gap-4 mb-4">
        ...
        <div className="bg-green-50 p-3 rounded-lg">
          ...
          <p className="text-xl font-bold text-green-800">
            ₹{pagination.total_amount ? Number(pagination.total_amount).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '0.00'}
          </p>
        </div>
        <div className="bg-purple-50 p-3 rounded-lg">
          <p className="text-sm text-purple-600 font-medium">Active Bids</p>
          <p className="text-xl font-bold text-purple-800">
            {filteredBids.filter((bid) => bid.status === 'active').length}
          </p>
        </div>
      </div>
            ...
            {currentBids.map((bid) => (
              ...
              >
                <td className="py-3 px-2 text-sm">
                  <div className="text-red-500 font-semibold">{getVillageName(bid.village)}</div>
                  <div className="text-xs text-gray-500 font-medium">{bid.full_name}</div>
                </td>
                <td className="py-3 px-2 text-sm text-gray-600">
                  {getGameName(bid.game_name)}
                </td>
                <td className="py-3 px-2 text-sm text-gray-600">
                  {bid.session_type}
                </td>
                ...
                <td className="py-3 px-2 text-sm font-mono font-bold text-blue-600">
                  {bid.bid_number}
                </td>
                ...
                <td className="py-3 px-2 text-center">
                  <span
                    className={`px-2 py-1 capitalize rounded-full text-xs font-medium ${getStatusColor(bid.status)}`}
                  >
                    {bid.status}
                  </span>
                </td>
                <td className="py-3 px-2 text-sm text-center text-gray-500">
                  {bid.winning_amount ? `₹${bid.winning_amount}` : '-'}
                </td>
                <td className="py-3 px-2 text-center">
                  <button
